...
def ComputeNorm(u,v,p,u_old,v_old,p_old,L2u,L2v,L2p):
    ...
    for i in range(1,m):
        for j in range(1,n):
            L2u+=(u[i,j]-u_old[i,j])**2
            L2v+=(v[i,j]-v_old[i,j])**2
            L2p+=(p[i,j]-p_old[i,j])**2
            num+=1           
    L2u=(L2u/num)**0.5
    L2v=(L2v/num)**0.5
    L2p=(L2p/num)**0.5   
...
while (L2u>tol or L2v>tol or L2p >tol) and it<iter_max:
    ...
    L2u,L2v,L2p=ComputeNorm(u,v,p,u_old,v_old,p_old,L2u,L2v,L2p)  
